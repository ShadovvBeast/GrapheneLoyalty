...
function sendCustomerMessage(customerId){
    ...
    var data = 'token=' + token_admin_customers + '&tab=AdminCustomers&ajax=1&action=sendCustomerMessage&id_customer=' + customerId + '&message=' + encodeURIComponent(messageContent);
    $.ajax({
        type: "POST",
        url: "index.php",
        data: data,
        ...
        success: function(r) {
            ...
            if (r == 'ok') {
                $('#submitCustomerNote').attr('disabled', true);
                showSuccessMessage(message_success_msg);
            }
...
function sendGroupMessage(groupId){
    var messageContent = $('#messageContent').val();
    var data = 'token=' + token_admin_groups + '&tab=AdminGroups&ajax=1&action=sendGroupMessage&id_group=' + groupId + '&message=' + encodeURIComponent(messageContent);
    $.ajax({
        type: "POST",
        url: "index.php",
        data: data,
