...
class AdminCustomersControllerCore extends AdminController
{
    ...
    public function renderForm()
    {
        ...
        $this->fields_form['input'] = array_merge(
            ...
            array(
                ...
                array(
                    'type' => 'text',
                    'label' => $this->l('NFC Identification'),
                    'class' => 'nfcinput',
                    'name' => 'nfc',
                    'readonly' => 'true',
                    'col' => '4',
                    'hint' => $this->l('Automatically supplied')
                ),
                array(
                    'type' => 'text',
                    ...
                    'name' => 'phone',
                    'col' => '4',
                    'hint' => $this->l('Customer phone number')
                )
            )
        );
     ...
     * @var Services_Twilio $client
     * @var Customer $customer
     * @return void
     */
    public function ajaxProcessSendCustomerMessage()
    {
        require('Services/Twilio.php');
        //if ($this->tabAccess['edit'] === '1') {
        $message = Tools::htmlentitiesDecodeUTF8(Tools::getValue('message'));
        $customer = new Customer((int)Tools::getValue('id_customer'));
        $account_sid = 'AC7a06eaf4ff664194008184893214c201';
        $auth_token = 'f75a524f2d71059665f36e1a05bbba14';
        $client = new Services_Twilio($account_sid, $auth_token);
        if ($customer->phone) {
            $phone = Tools::htmlentitiesDecodeUTF8($customer->phone);
            $message = $client->account->messages->sendMessage(
                '6473616365', // From a valid Twilio number
                $phone, // Text this number
                $message
            );
            die('ok');
        }
        die('no phone for customer');
        //print $message->sid;
